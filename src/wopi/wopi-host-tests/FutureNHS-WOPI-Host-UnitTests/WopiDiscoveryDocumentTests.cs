...
namespace FutureNHS_WOPI_Host_UnitTests
{
    ...
    public sealed class WopiDiscoveryDocumentTests
    {
        ...
        public async Task IsProofInvalid_ProofsAreCorrectlyIdentifiedAsEitherVaildOrInvalid(
            ...
            )
        {
            ...
            var wopiDiscoveryDocumentXml =
              "<wopi-discovery>" +
                "<proof-key exponent=\"" + exponent + "\" " +
                           "modulus=\"" + modulus + "\" " +
                           "value=\"" + proofKey + "\" " +
                           "oldexponent=\"" + oldExponent + "\" " +
                           "oldmodulus=\"" + oldModulus + "\" " +
                           "oldvalue=\"" + oldProofKey + "\" " +
                "/>" +
              "</wopi-discovery>";
            ;
            ...
            var httpResponseMessage = new HttpResponseMessage(System.Net.HttpStatusCode.OK)
            {
                Content = new StringContent(wopiDiscoveryDocumentXml, Encoding.UTF8, "application/xml")
            };
